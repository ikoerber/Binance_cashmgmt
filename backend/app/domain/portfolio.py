...
def compute_portfolio_from_ledger(
    ...
    for event in events:
        if event.type == EventType.TRADE_FILL:
            if event.side == TradeSide.BUY:
                # Buy: Base-Bestand erhöht, Kosten erhöht, EUR reduziert
                ...
                if event.fee_asset == base_asset and event.fee_amount and event.price:
                    # Base-Asset Fee: Menge um Fee reduzieren, KEINE zusätzlichen EUR-Kosten
                    base_qty += event.amount - event.fee_amount
                elif event.fee_asset == "EUR" and event.fee_amount:
                    # EUR Fee: volle Base-Menge, zusätzliche EUR-Kosten
                    base_qty += event.amount
                    trade_cost += event.fee_amount
                else:
                    # BNB oder andere Fee: Base-Menge nicht betroffen, aber EUR-Kosten erhoehen
                    base_qty += event.amount
                    if event.fee_eur_value and event.fee_asset not in ("EUR", base_asset, None):
                        trade_cost += event.fee_eur_value

                base_cost_basis_eur += trade_cost
                eur_available -= trade_cost

            elif event.side == TradeSide.SELL:
                # Sell: Base-Bestand reduziert, EUR erhöht, P&L realisiert
                ...
                # Fee in Base-Asset
                    ...
                    base_fee_eur = event.fee_amount * event.price
                    sell_proceeds -= base_fee_eur
                ...
                if base_qty > 0:
                    avg_cost = base_cost_basis_eur / base_qty
                ...
                # Bestände anpassen
                base_qty -= sell_qty
                base_cost_basis_eur -= cost_of_sold
                eur_available += sell_proceeds

        elif event.type == EventType.DEPOSIT:
            # Einzahlung (EUR oder Base-Asset)
            if event.asset == "EUR":
            ...
            # Auszahlung (EUR oder Base-Asset)
            if event.asset == "EUR":
                eur_available -= event.amount
            elif event.asset == base_asset:
                ...
                if base_qty > 0:
                    ...
                    base_cost_basis_eur -= cost_of_withdrawn
                base_qty -= event.amount
    ...
    return PortfolioState(
        ...
        base_qty=base_qty,
        base_cost_basis_eur=base_cost_basis_eur,
