...
## Milestone: v3.2 — UX Improvements

**Shipped:** 2026-02-28
**Phases:** 4 (23-26) | **Plans:** 5 | **Commits:** 17
**Requirements:** 10/10 satisfied

### What Was Built
- Navigation Structure: All pages (Settings, Status, Backtest) inside SymbolLayout with consistent 4-group sub-nav, legacy bookmark redirects via localStorage
- Dynamic Symbol Visibility: Backend active-symbols endpoint (Binance balance API), useActiveSymbols hook with WebSocket invalidation, GlobalNav + Overview + SymbolLayout filtered
- Dedicated Chart Page: Full-page OHLCV candlestick chart (lightweight-charts) with 5 intervals (15m/1h/4h/1d/1w), 4 lookback presets, volume histogram, 30s live polling
- Chart Overlays: 4 overlay types (orderblock zones, break-even, sell orders, trailing stop) with distinct visual styles, all individually toggleable

### What Worked
- Clean dependency chain: Phase 23 (layout) → 24 (filtering) → 25 (chart) → 26 (overlays) — each phase built cleanly on the previous
- Reusing existing API endpoints (orderblock candles, portfolio, orders, trailing stops) — zero new backend routes needed for overlays
- lightweight-charts `createPriceLine` API proved flexible enough for all 4 overlay types without custom primitives
- Sell order clustering (0.5% proximity threshold) solved the visual clutter problem elegantly
- 4 phases in a single day with zero gaps at milestone audit — research-skipped phases (pure frontend) can be fast when patterns are established

### What Was Inefficient
- SUMMARY.md one-liner extraction still returns N/A (3rd milestone with this issue) — tool format needs fixing
- Phase plan-index tool parsed both 26-01 and 26-02 as wave 1 despite frontmatter saying wave 2 — had to override from plan file reading
- Previous milestone audit (pre-Phase 26) incorrectly included Phases 27-28 in v3.2 scope, showing gaps_found — milestone scope determination needs tighter coupling to phase list

### Patterns Established
- **Overlay via paired price lines**: lightweight-charts lacks native rectangles — use two createPriceLine calls (zone_top + zone_bottom) with matching style
- **Conviction-based visual weight**: Map conviction levels to line width (1/2/3) instead of labels — keeps chart clean
- **Clustering for line overlays**: Group nearby values (0.5% threshold) into single visual element with count badge
- **Shared overlayLinesRef cleanup**: Single ref array for all overlay types, cleaned up in both effect body (before redraw) and cleanup return (on unmount)

### Key Lessons
1. Frontend-only milestones (no new backend routes) complete significantly faster — all complexity is in rendering and state, not API design
2. Phase dependency chains work best when each phase produces a clear artifact the next phase extends (SymbolLayout → useActiveSymbols → Chart.jsx → overlays)
3. The plan-index tool's wave parsing should be cross-checked against actual frontmatter when discrepancies appear
4. Milestone audit scope must match the `phases list` output, not all phases in ROADMAP.md

### Cost Observations
- Model mix: ~75% opus (executors), ~25% sonnet (verifiers, checkers)
- v3.2 completed in 1 calendar day (2026-02-28)
- Notable: 5 plans in ~18min total execution — fastest per-plan average across all milestones
